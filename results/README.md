...
| `fhe_fides_real_d768_t103_block0_simd_full_t103_scheme_b_a100_20260730` | Complete packed B=8 Token-SIMD T=103 block 0 | `[V]` pass, global `4.78e-9`, 156 products vs 1236 serial-equivalent, `6281.75s` encrypted evaluation; `[U]` timing heavily co-tenant-contaminated |
| `fhe_fides_real_d768_t103_block0_simd_full_t103_scheme_b_vram_a100_20260730` | PID-specific memory telemetry for the complete packed T=103 block | `[V]` target process peaks at `12920 MiB`; `[U]` one block, not the 12-block graph |
...
**Token-SIMD: complete T=103 block now proven.** The B=8 packed LN1-plus-query micro-gate and its same-source serial control first established the structural signal: dense calls `103 -> 13` (`7.923x`), observed encrypted-evaluation `2260.08s -> 306.54s` (`7.3728x`), and target-PID peak `15992 -> 9848 MiB`. The follow-on complete packed T=103 block now passes the full LN1/QKV/causal-softmax/attention-projection/LN2/GELU/MLP/residual graph at `global_rel_inf=4.78e-9`. Its exact dense-call count is `156` versus `1236` serial-equivalent, encrypted evaluation is `6281.75s`, and target-PID peak is `12920 MiB`. `[U]` Every timing run acquired co-tenants; the complete-block GPU later reached `76757 MiB` total with a `65868 MiB` cotenant, so uncontended throughput remains unknown. `[U]` All 12 blocks and the task head remain untested at T=103.
