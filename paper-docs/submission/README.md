...
Built from `paper-docs/manuscript/` as of 2026-09-08 (post `f1774ab` "JBHI restructuring", plus the
`fig_waterfall`/`fig_cost_split` → `fig_cost_decomposition` merge, eight figures total). This directory
...
LaTeX Live 2022 through Lmod, not on `PATH` by default) compiles clean — 12 pages, no undefined
...
To compile and visually check the manuscript itself before rebuilding the zip (recommended after any
`manuscript/` change): `module load texlive/2023 && paper-docs/scripts/build.sh all`, then open
`paper-docs/manuscript/source/main.pdf` (gitignored build artifact, not part of the zip).

## Status per required field

| Portal field | File | Status |
|---|---|---|
| Main Manuscript | `DNAGPT-FHE_main_manuscript_source.zip` | Ready, and compiles clean (verified 2026-09-08, see above). LaTeX source bundle (`main.tex`, `preamble.tex`, `refs.bib`, `sections/*.tex`, `figures/*.pdf`) — no build artifacts (`.aux`/`.bbl`/`.pdf` etc.) included, matches the portal's "bundle LaTeX files in a single archive" option. **Not yet ready to submit**: `00_frontmatter.tex` still carries `TODO(authors)` placeholders for institutional e-mails, corresponding-author designation, and funding/support text (see `paper-docs/TODO.md`). |
