...
then an expanded MLP with GELU and another residual. The dense maps use fixed model weights, whereas
LayerNorm, attention, and GELU introduce data-dependent products or nonlinear functions.

The release includes fine-tuned heads for genomic-signal recognition and mRNA abundance regression.
It does not include a head for the genome-understanding benchmark, so Section~\ref{sec:baseline}
evaluates that backbone with a locally fine-tuned linear head. Each of these families predates the
model: signal recognition by task-specific convolutional architectures~\cite{deepgsr}, abundance
regression likewise~\cite{xpresso}, and the understanding benchmark by the encoder-based model that
introduced it~\cite{dnabert2}. They first establish model fidelity; the encrypted study then uses
the signal-recognition path as its task-length anchor.
...
The alternative is to place exact functions at declared client boundaries. The key holder decrypts
only values derived from its query, evaluates the nonlinearity, and returns a fresh ciphertext. This
removes approximation-range calibration and resets the encrypted depth budget, but it requires an
online client and exposes intermediate activations to that client. The choice between polynomial
approximation and client evaluation therefore determines memory, interaction, depth, and the threat
model. Section~\ref{sec:protocol} evaluates both designs and reports where each stops.
...
We also evaluated a Boolean/integer-oriented backend whose transformer protocol places
nonlinearities at the client. Its available GPU acceleration and ciphertext expansion were less
suitable for this matrix-heavy graph than the already validated CKKS
path~\cite{concreteml,tfhers}. A
separate CKKS wrapper was not selected because its public interface lacked the refresh and general
nonlinear-function support required by the non-interactive baseline. These decisions select among
the software paths tested in this project. They do not establish that CKKS is the only scheme able
to express encrypted transformer inference.
