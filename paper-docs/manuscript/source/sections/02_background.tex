...
DNA~\cite{dnagpt2023}. We evaluate its $0.1$-billion-parameter configuration. The backbone has
...
the budget in an interactive protocol. The latter is used by the client-assisted protocol in this
study, which therefore requires no homomorphic bootstrapping. Both choices carry system costs, so
multiplicative depth is an architectural resource rather than only a cryptographic parameter.
