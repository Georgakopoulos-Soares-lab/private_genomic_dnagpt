...
retains the secret key; the compute provider evaluates the served model on those ciphertexts.
...
The evaluated input path assumes that the provider distributes the tokenizer and embedding table
so that the data owner can construct embedded vectors locally. This is compatible with the public
artifact used in the experiment, but it is an explicit service assumption. A provider that will
not distribute those components would instead need a private token-index lookup, which is outside
...
complete encrypted inference reported in Section~\ref{sec:results}, the data owner spends
$956.187$~s evaluating LayerNorm inverse square roots, causal softmax, and GELU at the boundaries.
This study records no comparable local CPU forward-pass time, so it does not quantify the overhead.
The evaluated weights are also public, and no model-confidentiality argument requires the service
arrangement.

The protocol instead addresses a served-model deployment in which the client may not hold the
weights. Under that premise, the choices are to disclose the genome, decline the inference, or use
a privacy-preserving protocol. Client-assisted CKKS provides access to the served model without
genomic disclosure; it is not intended to outperform plaintext inference.
...
accelerator hardware, while the key holder evaluates the exact nonlinearities on CPU. This
allocation protects the genomic input without requiring the data owner to operate a GPU.
