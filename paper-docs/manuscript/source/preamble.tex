...
% Latin Modern before fontenc: the default CM fonts have no T1 bold small-caps, which
% the \scshape\bfseries section headings below request. Without this the engine
% substitutes silently and the headings lose their weight.
\usepackage{lmodern}
\usepackage[T1]{fontenc}
% Latin Modern ships bold small-caps as series "b", not the "bx" that \bfseries
% requests, so map one onto the other rather than let the engine substitute.
