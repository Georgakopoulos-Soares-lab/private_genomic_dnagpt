...
if [[ ! -d "${FIDES_ROOT}/.git" ]]; then
  echo "[FATAL] FIDESlib git checkout missing at ${FIDES_ROOT}" >&2
  exit 2
fi
...
cmake --build "${BUILD_DIR}" --target dnagpt_packed_t8_fides --parallel "${JOBS}"
echo "[build] ${BUILD_DIR}/dnagpt_packed_t8_fides"
