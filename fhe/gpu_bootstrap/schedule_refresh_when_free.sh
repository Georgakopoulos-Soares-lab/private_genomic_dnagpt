...
if [[ $# -ne 8 ]]; then
  echo "usage: $0 GPU_CSV REMOTE_ROOT SOURCE_SUBDIR ACTIVATION_SUBPATH IMAGE_TAG MODE RUN_TAG MAX_POLLS" >&2
  exit 2
fi
...
readonly LAUNCHER="${SCRIPT_DIR}/launch_brev_refresh.sh"
  ...
  for gpu in "${GPUS[@]}"; do
    ...
    if (( stable_polls["${gpu}"] >= REQUIRED_STABLE_POLLS )); then
      ...
      if "${LAUNCHER}" \
        ...
        "${MODE}" \
        "${RUN_TAG}"; then
        readonly RUN_DONE="${REMOTE_ROOT}/${SOURCE_SUBDIR}/fhe/gpu_bootstrap/${RUN_TAG}.done"
        while [[ ! -e "${RUN_DONE}" ]]; do
          sleep "${POLL_SECONDS}"
        done
        exit 0
