...
if [[ $# -ne 7 ]]; then
  echo "usage: $0 PHYSICAL_GPU REMOTE_ROOT SOURCE_SUBDIR ACTIVATION_SUBPATH IMAGE_TAG MODE RUN_TAG" >&2
  exit 2
fi
...
readonly ACTIVATION_SUBPATH="$4"
readonly IMAGE_TAG="$5"
readonly MODE="$6"
readonly RUN_TAG="$7"
readonly MODULE_DIR="${REMOTE_ROOT}/${SOURCE_SUBDIR}/fhe/gpu_bootstrap"
...
if [[ "${MODE}" != "native-gpu" && "${MODE}" != "cpu-iterative-interop" ]]; then
  echo "[FATAL] unsupported refresh mode: ${MODE}" >&2
  exit 2
fi
if [[ "${RUN_TAG}" != *"_refresh_"* ]]; then
  echo "[FATAL] refresh evidence tag must contain _refresh_" >&2
  exit 2
fi
...
if [[ ! -f "${REMOTE_ROOT}/${ACTIVATION_SUBPATH}" ]]; then
  echo "[FATAL] activation fixture missing: ${REMOTE_ROOT}/${ACTIVATION_SUBPATH}" >&2
  exit 2
fi

readonly IMAGE_EVIDENCE="${IMAGE_TAG}@sha256:8dfa77fa298472824a86414efdc6a5d14c4fa57bce3447b61b9893fe37d547a4"
export PHYSICAL_GPU REMOTE_ROOT SOURCE_SUBDIR ACTIVATION_SUBPATH
export IMAGE_TAG IMAGE_EVIDENCE MODE RUN_TAG LOG DONE
nohup sh -c '
  docker run --rm \
    --gpus "device=${PHYSICAL_GPU}" \
    -v "${REMOTE_ROOT}:/work" \
    -w "/work/${SOURCE_SUBDIR}/fhe/gpu_bootstrap" \
    -e "FIDES_REFRESH_BUILD_DIR=/work/${SOURCE_SUBDIR}/fhe/gpu_bootstrap/build" \
    -e "FIDES_CONTAINER_IMAGE=${IMAGE_EVIDENCE}" \
    -e "FIDES_RUN_ENVIRONMENT=Brev A100-SXM4-80GB physical GPU ${PHYSICAL_GPU} [gpu]" \
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/fhe/gpu_bootstrap/run_refresh.sh" \
    0 \
    "${MODE}" \
    "/work/${ACTIVATION_SUBPATH}" \
    "/work/${SOURCE_SUBDIR}/fhe/gpu_bootstrap/evidence/${RUN_TAG}.json" \
    >"${LOG}" 2>&1
  status=$?
  printf "%s\n" "${status}" >"${DONE}"
' >/dev/null 2>&1 &
