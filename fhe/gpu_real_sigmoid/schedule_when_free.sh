...
if [[ $# -ne 8 ]]; then
  echo "usage: $0 GPU_CSV REMOTE_ROOT SOURCE_SUBDIR FIXTURE_SUBDIR IMAGE_TAG GATE RUN_TAG MAX_POLLS" >&2
  exit 2
fi
...
readonly GATE="$6"
readonly RUN_TAG="$7"
readonly MAX_POLLS="$8"
readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly LAUNCHER="${SCRIPT_DIR}/launch_brev_sigmoid.sh"
...
if [[ "${RUN_TAG}" != *"_sigmoid13_"* ]]; then
  echo "[FATAL] optimized evidence tag must contain _sigmoid13_" >&2
  exit 2
fi
if [[ ! -x "${LAUNCHER}" ]]; then
  ...
  for gpu in "${GPUS[@]}"; do
    ...
    if (( stable_polls["${gpu}"] >= REQUIRED_STABLE_POLLS )); then
      ...
      if "${LAUNCHER}" \
        ...
        "${GATE}" \
        "${RUN_TAG}"; then
        readonly RUN_DONE="${REMOTE_ROOT}/${SOURCE_SUBDIR}/${RUN_TAG}.done"
        while [[ ! -e "${RUN_DONE}" ]]; do
          sleep "${POLL_SECONDS}"
        done
        exit 0
      ...
      status=$?
      echo "launcher_refused status=${status}; resuming clean-GPU search"
      rmdir "${CURRENT_LOCK}"
      CURRENT_LOCK=""
      stable_polls["${gpu}"]=0
    fi
