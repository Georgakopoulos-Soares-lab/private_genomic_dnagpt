#!/usr/bin/env bash
# Orchestrator for the 2-GPU process-per-GPU sharding Stage 1 (Q/K/V split)
# micro-gate: waits for host capacity, launches the Token-SIMD-parameter-
# matched shard writer (builds context/keys, serializes to a fresh state
# directory, exits without evaluating), waits for it to finish, then waits
# for capacity on TWO distinct physical GPUs simultaneously and launches two
# independent shard-reader workers concurrently -- worker A with
# `--part query,key`, worker B with `--part value` -- both pointed
# read-only at the same state directory. Deletes the state directory
# afterward regardless of pass/fail (it contains a serialized secret-key
# file; see the writer/reader C++ source header comments and
# ~/.agents/policies/secrets.md for the hygiene rationale).
#
# Mirrors wait_and_run_scheme_b_serialize.sh's capacity-wait/cleanup logic
# (same LOAD_THRESHOLD/POLL_SECONDS/MAX_WAIT_SECONDS defaults, same
# stderr-only log() inside wait_for_capacity so poll lines never corrupt a
# stdout GPU-index capture -- see wait_and_run_scheme_b.sh's header comment
# for the 2026-07-25 incident this avoids), generalized to wait for two
# simultaneously-idle GPUs before the reader phase instead of one. Runs
# directly on the Brev host (not inside a container) since it needs
# host-level `uptime`/`nvidia-smi` and drives the launch_brev_*.sh scripts,
# which themselves launch the actual work in detached `docker run`
# invocations.
#
...
readonly READER_A_TAG="${SCHEME_B_SHARD_READER_A_RUN_TAG:?set SCHEME_B_SHARD_READER_A_RUN_TAG (must contain _scheme_b_ and _qkvshard_)}"
readonly READER_B_TAG="${SCHEME_B_SHARD_READER_B_RUN_TAG:?set SCHEME_B_SHARD_READER_B_RUN_TAG (must contain _scheme_b_ and _qkvshard_)}"
...
readonly ORCHESTRATOR_LOG="${SOURCE_DIR}/${WRITER_TAG}.orchestrator.log"
...
run_writer_phase() {
  ...
  log "launching writer phase tag=${WRITER_TAG} gpu=${gpu}"
...
# Launches BOTH shard-reader workers back-to-back (each launch script itself
# detaches into its own nohup'd docker run and returns immediately, so this
# is not a blocking pair) and then waits for both .done files. This is the
# concurrency point of Stage 1: two independent FIDESlib/CUDA processes,
# each pinned to a different physical GPU, sharing one deserialized
# context/key lineage.
run_reader_phase_pair() {
  ...
  log "launching reader worker A tag=${READER_A_TAG} gpu=${gpu_a} parts=${READER_A_PARTS}"
  if ! SCHEME_B_SHARD_READER_RUN_TAG="${READER_A_TAG}" SCHEME_B_IMAGE="${IMAGE}" \
      "${SOURCE_DIR}/launch_brev_scheme_b_simd_shard_reader_t103_depth13.sh" "${gpu_a}" \
  ...
  log "launching reader worker B tag=${READER_B_TAG} gpu=${gpu_b} parts=${READER_B_PARTS}"
  if ! SCHEME_B_SHARD_READER_RUN_TAG="${READER_B_TAG}" SCHEME_B_IMAGE="${IMAGE}" \
      "${SOURCE_DIR}/launch_brev_scheme_b_simd_shard_reader_t103_depth13.sh" "${gpu_b}" \
      "${REMOTE_ROOT}" "${SOURCE_SUBDIR}" "${STATE_SUBDIR}" "${FIXTURE_SUBDIR}" \
      "${READER_B_PARTS}" \
      >>"${ORCHESTRATOR_LOG}" 2>&1; then
...
main() {
  log "shard-qkv orchestrator starting (pid $$) writer_tag=${WRITER_TAG} reader_a_tag=${READER_A_TAG} reader_b_tag=${READER_B_TAG}"
  ...
  if [[ -f "${SOURCE_DIR}/evidence/${READER_A_TAG}.json" && \
    ...
    cleanup_state_dir
    log "SCHEME B TOKEN-SIMD SHARD-QKV STAGE-1 RUN COMPLETE"
    exit 0
  fi
