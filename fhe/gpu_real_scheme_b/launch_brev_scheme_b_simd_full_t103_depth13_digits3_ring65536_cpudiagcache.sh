...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || \
  ...
  echo "[FATAL] depth-13/digits-3/ring-65536 CPU-diagcache SIMD tag must contain _scheme_b_ and _simd_full_t103_depth13_digits3_ring65536_cpudiagcache_" >&2
  exit 2
fi
...
nohup sh -c '
  ...
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_simd_full_t103_depth13_digits3_ring65536_cpudiagcache.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
