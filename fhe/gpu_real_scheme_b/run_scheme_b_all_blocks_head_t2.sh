...
readonly EXPECTED_FROZEN_SOURCE=d88f1a0919a003a539e746aaf33e5d283c28810c2805a1af4b05dffac43e08df
readonly EXPECTED_FIXTURE_CONTRACT=c72dd1a897cf2dc66768a1bbd077b40fb4e81c7c72247e811cdcb71465b488de
...
if [[ $# -ne 3 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain _scheme_b_ and _all_blocks_head_t2_" >&2
  exit 2
fi
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_all_blocks_head_t2"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_all_blocks_head_t2_"* ]]; then
  echo "[FATAL] all-block evidence filename is missing a required identity token" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
contract_sha="$(sha256sum "${SCRIPT_DIR}/fixture_all_blocks_head_t2.sha256" | cut -d' ' -f1)"
...
(
  cd "${FIXTURE_DIR}"
  sha256sum --check --status "${SCRIPT_DIR}/fixture_all_blocks_head_t2.sha256"
)
...
frozen_source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b.cpp" | cut -d' ' -f1)"
if [[ "${frozen_source_sha}" != "${EXPECTED_FROZEN_SOURCE}" ]]; then
  echo "[FATAL] frozen Scheme B source ${frozen_source_sha}; expected ${EXPECTED_FROZEN_SOURCE}" >&2
  exit 2
fi
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_all_blocks_head_t2.cpp" | cut -d' ' -f1)"
...
exec "${BINARY}" \
  ...
  --frozen-source-sha256 "${frozen_source_sha}" \
