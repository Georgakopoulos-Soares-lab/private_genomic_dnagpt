...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_general_attention_t32_"* ]]; then
  echo "[FATAL] general-attention T=32 Scheme B evidence tag must contain both _scheme_b_ and _general_attention_t32_" >&2
  exit 2
fi

# Same rationale as launch_brev_scheme_b_general_attention_t8.sh: detached
# nohup+.done-sentinel launch so a local SSH/session disconnect during this
# (longer than T=8: 496 attention round trips vs. 28) run cannot kill the
# remote docker process or trigger a reconnect-triggered double-launch (see
# docs/hybrid/tasks.md's 2026-07-28 T=8 growth-trend entry for the incident
# that motivated this pattern -- carried forward unchanged for T=32).
...
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_general_attention_t32.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
