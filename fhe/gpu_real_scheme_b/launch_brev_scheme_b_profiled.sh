...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_profiled_"* ]]; then
  echo "[FATAL] profiled Scheme B evidence tag must contain both _scheme_b_ and _profiled_" >&2
  exit 2
fi
...
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_profiled.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
