...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 GPU STATE_DIR FIXTURE_DIR OUTPUT_JSON" >&2
...
readonly OUTPUT="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_serialize_reader"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_serialized_"* ]]; then
  echo "[FATAL] serialize-reader evidence filename must contain both _scheme_b_ and _serialized_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
if [[ ! -d "${STATE_DIR}" ]]; then
  echo "[FATAL] state directory missing (writer phase must run first): ${STATE_DIR}" >&2
  exit 2
fi
if [[ ! -f "${STATE_DIR}/writer_timing.txt" ]]; then
  echo "[FATAL] ${STATE_DIR}/writer_timing.txt missing -- writer phase did not complete" >&2
  exit 2
fi
if [[ ! -x "${BINARY}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_serialize_reader.cpp" | cut -d' ' -f1)"
