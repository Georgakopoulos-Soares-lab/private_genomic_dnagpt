...
if [[ $# -ne 3 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _two_block_refresh_" >&2
  exit 2
fi
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_two_block_refresh"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_two_block_refresh_"* ]]; then
  echo "[FATAL] two-block Scheme B evidence filename must contain both _scheme_b_ and _two_block_refresh_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_two_block_refresh.cpp" | cut -d' ' -f1)"
