...
readonly EXPECTED_MANIFEST=bfdabe62cdc7ff05dfb3e6ca149e50b081d5cfeb159408bb7478f2b168abd5c3
...
if [[ $# -ne 4 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _general_attention_" >&2
...
readonly OUTPUT="$3"
readonly GATE="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_general_attention"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_general_attention_"* ]]; then
  echo "[FATAL] general-attention Scheme B evidence filename must contain both _scheme_b_ and _general_attention_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
(
  cd "${FIXTURE_DIR}"
  sha256sum --check --status "${SCRIPT_DIR}/fixture_t3.sha256"
)
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_general_attention.cpp" | cut -d' ' -f1)"
contract_sha="$(sha256sum "${SCRIPT_DIR}/fixture_t3.sha256" | cut -d' ' -f1)"
