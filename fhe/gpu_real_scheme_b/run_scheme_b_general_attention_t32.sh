...
readonly EXPECTED_MANIFEST=6ae018b419a533dd9c77e6e0415bf7adf2b67c1f444013e68489a2fc84cca003
...
if [[ $# -ne 4 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _general_attention_t32_" >&2
...
readonly OUTPUT="$3"
readonly GATE="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_general_attention_t32"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_general_attention_t32_"* ]]; then
  echo "[FATAL] general-attention T=32 Scheme B evidence filename must contain both _scheme_b_ and _general_attention_t32_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
(
  cd "${FIXTURE_DIR}"
  sha256sum --check --status "${SCRIPT_DIR}/fixture_t32.sha256"
)
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_general_attention_t32.cpp" | cut -d' ' -f1)"
contract_sha="$(sha256sum "${SCRIPT_DIR}/fixture_t32.sha256" | cut -d' ' -f1)"
