...
readonly EXPECTED_SOURCE=6e8cd08efad1567d2f9001f69d10e302e45f4e634f3bd37e2245382d215fbe5e
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_simd_full_t103_depth13_digits3_ring65536"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_simd_full_t103_depth13_digits3_ring65536_"* ]]; then
  echo "[FATAL] depth-13/digits-3/ring-65536 SIMD evidence filename must contain _scheme_b_ and _simd_full_t103_depth13_digits3_ring65536_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
