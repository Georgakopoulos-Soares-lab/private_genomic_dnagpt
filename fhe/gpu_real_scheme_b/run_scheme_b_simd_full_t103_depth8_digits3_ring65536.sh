...
readonly EXPECTED_SOURCE=65cc818cbb299af984cccdbe6a7df891cac5b7071f663edfac8c3f13be161365
readonly EXPECTED_PARENT=c53e47123614bcddf6a577ca5cc1e22db786d234e953e5c31b6a488e3408c089
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_simd_full_t103_depth8_digits3_ring65536"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_simd_full_t103_depth8_digits3_ring65536_"* ]]; then
  echo "[FATAL] depth-8/digits-3/ring-65536 SIMD evidence filename must contain _scheme_b_ and _simd_full_t103_depth8_digits3_ring65536_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth8_digits3_ring65536.cpp" | cut -d' ' -f1)"
if [[ "${source_sha}" != "${EXPECTED_SOURCE}" ]]; then
  echo "[FATAL] depth-8/digits-3/ring-65536 full SIMD source ${source_sha}; expected ${EXPECTED_SOURCE}" >&2
  exit 2
fi
parent_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth8_digits3.cpp" | cut -d' ' -f1)"
