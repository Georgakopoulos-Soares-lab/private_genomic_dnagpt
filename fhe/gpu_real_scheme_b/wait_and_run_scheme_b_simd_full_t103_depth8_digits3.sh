#!/usr/bin/env bash
# Capacity-gated T=103 Token-SIMD depth-8/digits-3 complete block-0 gate.
set -uo pipefail
...
readonly RUN_TAG="${SCHEME_B_SIMD_FULL_DEPTH8_DIGITS3_RUN_TAG:?set SCHEME_B_SIMD_FULL_DEPTH8_DIGITS3_RUN_TAG}"
...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || \
   [[ "${RUN_TAG}" != *"_simd_full_t103_depth8_digits3_"* ]]; then
  echo "[FATAL] bad depth-8/digits-3 SIMD full-block tag ${RUN_TAG}" >&2
  exit 2
fi
...
run_gate() {
  ...
  while true; do
    ...
    log "launching depth-8/digits-3 SIMD full block tag=${RUN_TAG} gpu=${gpu}"
    if ! "${SOURCE_DIR}/launch_brev_scheme_b_simd_full_t103_depth8_digits3.sh" \
    ...
    log "depth-8/digits-3 SIMD full-block process exit status=${rc}"
    if [[ "${rc}" -ne 0 ]]; then
      return "${rc}"
    fi
    return 0
...
main() {
  log "depth-8/digits-3 SIMD full-block orchestrator starting (pid $$)"
  if ! run_gate; then
    log "FATAL: SIMD full-block gate failed"
    exit 1
  fi
  log "SCHEME B SIMD FULL T103 DEPTH8 DIGITS3 COMPLETE"
}
