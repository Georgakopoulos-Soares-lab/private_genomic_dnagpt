...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_diagcache_"* ]]; then
  echo "[FATAL] diagcache Scheme B evidence tag must contain both _scheme_b_ and _diagcache_" >&2
  exit 2
fi
...
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_diagcache.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
