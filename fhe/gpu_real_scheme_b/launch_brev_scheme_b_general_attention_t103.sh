...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_general_attention_t103_"* ]]; then
  echo "[FATAL] general-attention T=103 Scheme B evidence tag must contain both _scheme_b_ and _general_attention_t103_" >&2
  exit 2
fi

# Same rationale as the T=8/T=32 launchers: detached nohup+.done-sentinel
# launch so a local SSH/session disconnect during this (much longer than
# T=32: ~5476 attention-gate round trips vs 528, plus 3x the QKV/MLP matmul
# work) run cannot kill the remote docker process.
...
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_general_attention_t103.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
