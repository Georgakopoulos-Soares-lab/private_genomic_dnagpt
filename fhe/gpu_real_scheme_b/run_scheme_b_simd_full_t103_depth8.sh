...
readonly EXPECTED_SOURCE=58c257522eefe7a928e6efaa486af6dd3ac53c289fc07878bb41d5d5f9350882
readonly EXPECTED_PARENT=97727a5bb2145b32749b746c3fc1b576811dfcc3222088278962b029fc1aea4b
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_simd_full_t103_depth8"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_simd_full_t103_depth8_"* ]]; then
  echo "[FATAL] depth-8 SIMD evidence filename must contain _scheme_b_ and _simd_full_t103_depth8_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth8.cpp" | cut -d' ' -f1)"
if [[ "${source_sha}" != "${EXPECTED_SOURCE}" ]]; then
  echo "[FATAL] depth-8 full SIMD source ${source_sha}; expected ${EXPECTED_SOURCE}" >&2
  exit 2
fi
parent_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103.cpp" | cut -d' ' -f1)"
