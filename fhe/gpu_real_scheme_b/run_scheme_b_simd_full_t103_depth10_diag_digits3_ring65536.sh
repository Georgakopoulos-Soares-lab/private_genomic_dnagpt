...
readonly EXPECTED_SOURCE=258e0da1ece28ef8f42652097ed8c7b61218b2a76c2d297f879a860410c72e82
readonly EXPECTED_PARENT=703a72d8dcc76cbe93234e04dcc953cce7daf0b48b64011ac1239129333d5ca2
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_simd_full_t103_depth10_diag_digits3_ring65536"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_simd_full_t103_depth10_diag_digits3_ring65536_"* ]]; then
  echo "[FATAL] depth-10-diag/digits-3/ring-65536 SIMD evidence filename must contain _scheme_b_ and _simd_full_t103_depth10_diag_digits3_ring65536_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth10_diag_digits3_ring65536.cpp" | cut -d' ' -f1)"
if [[ "${source_sha}" != "${EXPECTED_SOURCE}" ]]; then
  echo "[FATAL] depth-10-diag/digits-3/ring-65536 full SIMD source ${source_sha}; expected ${EXPECTED_SOURCE}" >&2
  exit 2
fi
parent_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth10_digits3_ring65536.cpp" | cut -d' ' -f1)"
