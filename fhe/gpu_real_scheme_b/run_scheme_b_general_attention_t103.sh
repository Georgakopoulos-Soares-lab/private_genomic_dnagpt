...
if [[ $# -ne 4 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _general_attention_t103_" >&2
...
readonly OUTPUT="$3"
readonly GATE="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_general_attention_t103"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_general_attention_t103_"* ]]; then
  echo "[FATAL] general-attention T=103 Scheme B evidence filename must contain both _scheme_b_ and _general_attention_t103_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_general_attention_t103.cpp" | cut -d' ' -f1)"
