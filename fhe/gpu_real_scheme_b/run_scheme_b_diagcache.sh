...
if [[ $# -ne 3 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _diagcache_" >&2
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_diagcache"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_diagcache_"* ]]; then
  echo "[FATAL] diagcache Scheme B evidence filename must contain both _scheme_b_ and _diagcache_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_diagcache.cpp" | cut -d' ' -f1)"
