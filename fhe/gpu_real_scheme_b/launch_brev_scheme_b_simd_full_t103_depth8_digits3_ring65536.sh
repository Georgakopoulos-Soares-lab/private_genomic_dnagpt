...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || \
  ...
  echo "[FATAL] depth-8/digits-3/ring-65536 SIMD tag must contain _scheme_b_ and _simd_full_t103_depth8_digits3_ring65536_" >&2
  exit 2
fi
...
nohup sh -c '
  ...
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_simd_full_t103_depth8_digits3_ring65536.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
