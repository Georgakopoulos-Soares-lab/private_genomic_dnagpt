...
readonly EXPECTED_SOURCE=703a72d8dcc76cbe93234e04dcc953cce7daf0b48b64011ac1239129333d5ca2
readonly EXPECTED_PARENT=738da0c0c32bd14407ee14877925c6fe7e45fe17d6863f402d46f6bdf98ae235
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_simd_full_t103_depth10_digits3_ring65536"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_simd_full_t103_depth10_digits3_ring65536_"* ]]; then
  echo "[FATAL] depth-10/digits-3/ring-65536 SIMD evidence filename must contain _scheme_b_ and _simd_full_t103_depth10_digits3_ring65536_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth10_digits3_ring65536.cpp" | cut -d' ' -f1)"
if [[ "${source_sha}" != "${EXPECTED_SOURCE}" ]]; then
  echo "[FATAL] depth-10/digits-3/ring-65536 full SIMD source ${source_sha}; expected ${EXPECTED_SOURCE}" >&2
  exit 2
fi
parent_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_simd_full_t103_depth9_digits3_ring65536.cpp" | cut -d' ' -f1)"
