...
readonly OUTPUT="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b"

if [[ "${GATE}" != "ln1" && "${GATE}" != "attention" && "${GATE}" != "full" ]]; then
  echo "[FATAL] gate must be ln1, attention, or full" >&2
  exit 2
fi
if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]]; then
  echo "[FATAL] Scheme B evidence filename must contain _scheme_b_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b.cpp" | cut -d' ' -f1)"
