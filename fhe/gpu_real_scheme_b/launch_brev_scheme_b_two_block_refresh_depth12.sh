...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || \
   [[ "${RUN_TAG}" != *"_two_block_refresh_"* ]] || \
   [[ "${RUN_TAG}" != *"_depth12_"* ]]; then
  echo "[FATAL] depth-12 evidence tag is missing a required identity token" >&2
  exit 2
fi
...
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_two_block_refresh_depth12.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
