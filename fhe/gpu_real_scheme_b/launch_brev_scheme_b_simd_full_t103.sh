...
if [[ $# -ne 7 ]]; then
  echo "usage: $0 PHYSICAL_GPU REMOTE_ROOT SOURCE_SUBDIR FIXTURE_SUBDIR IMAGE_TAG RUN_TAG MODE" >&2
  exit 2
fi
...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || \
   [[ "${RUN_TAG}" != *"_simd_linear_t103_"* ]] || \
   [[ "${RUN_TAG}" != *"_${MODE}_"* ]]; then
  echo "[FATAL] SIMD tag must contain _scheme_b_, _simd_linear_t103_, and _${MODE}_" >&2
  exit 2
fi
...
export IMAGE_TAG RUN_TAG MODE LOG DONE VRAM_LOG PREFLIGHT_NOTE
...
nohup sh -c '
  ...
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_simd_linear_t103.sh" \
    0 \
    "/work/${FIXTURE_SUBDIR}" \
