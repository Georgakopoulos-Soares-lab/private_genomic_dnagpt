#!/usr/bin/env bash
# Orchestrator for the GPU-side profiling prototype
# (real_dnagpt_fides_scheme_b_lintransform): waits for host load + GPU capacity,
# then runs one profiled evaluation of the block-0 "full" gate unattended.
#
...
# `uptime`/`nvidia-smi` and drives launch_brev_scheme_b_lintransform.sh, which
...
# (launch_brev_scheme_b_lintransform.sh is fail-closed on that already).
set -uo pipefail

readonly REMOTE_ROOT="${SCHEME_B_REMOTE_ROOT:-/data/christos/private_genomic_ml/dnagpt_fides_asymfix2_20260724}"
readonly SOURCE_SUBDIR="${SCHEME_B_SOURCE_SUBDIR:-gpu_real_scheme_b_lintransform_v1/fhe/gpu_real_scheme_b}"
...
readonly RUN_TAG="${SCHEME_B_LINTRANSFORM_RUN_TAG:?set SCHEME_B_LINTRANSFORM_RUN_TAG (must contain _scheme_b_ and _lintransform_)}"
...
readonly ORCHESTRATOR_LOG="${SOURCE_DIR}/scheme_b_lintransform_orchestrator.log"
...
run_lintransform() {
  ...
  log "launching lintransform run tag=${tag} gpu=${gpu}"
  if ! "${SOURCE_DIR}/launch_brev_scheme_b_lintransform.sh" "${gpu}" "${REMOTE_ROOT}" \
      "${SOURCE_SUBDIR}" "${FIXTURE_SUBDIR}" "${IMAGE}" "${tag}" \
...
main() {
  log "lintransform orchestrator starting (pid $$) tag=${RUN_TAG}"
  ...
  while true; do
    ...
    if [[ "${rc}" -ne 0 ]]; then
      log "FATAL: lintransform run failed with exit ${rc}; stopping (manual investigation needed)"
      exit "${rc}"
    fi
    log "lintransform run PASSED"
    break
  done
  log "SCHEME B LINTRANSFORM RUN COMPLETE"
}
