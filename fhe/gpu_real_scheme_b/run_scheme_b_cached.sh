...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 GPU REPEATS(>=2) FIXTURE_DIR OUTPUT_JSON" >&2
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _cached_" >&2
  exit 2
fi
...
readonly OUTPUT="$4"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_cached"

if ! [[ "${REPEATS}" =~ ^[0-9]+$ ]] || (( REPEATS < 2 )); then
  echo "[FATAL] REPEATS must be an integer >= 2" >&2
  exit 2
fi
if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_cached_"* ]]; then
  echo "[FATAL] cached Scheme B evidence filename must contain both _scheme_b_ and _cached_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_cached.cpp" | cut -d' ' -f1)"
