...
readonly RUN_TAG="${SCHEME_B_SHARD_READER_RUN_TAG:?set SCHEME_B_SHARD_READER_RUN_TAG (must contain _scheme_b_ and _qkvshard_)}"
...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_qkvshard_"* ]]; then
  echo "[FATAL] shard-reader evidence tag must contain both _scheme_b_ and _qkvshard_" >&2
  exit 2
fi
...
nohup sh -c '
  ...
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_simd_shard_reader_t103_depth13.sh" \
    0 \
    "/work/${STATE_SUBDIR}" \
    "/work/${FIXTURE_SUBDIR}" \
