...
// GENERAL causal-attention prototype (T>2).
//
// Forked from the frozen real_dnagpt_fides_scheme_b.cpp (never edit that
// file). Every stage is byte-for-byte identical to the frozen file EXCEPT the
// attention block: the frozen file exploits a closed-form identity
// (softmax over exactly two causal columns reduces to a single sigmoid of a
// score difference) that only holds at T=2. This file replaces that identity
// with a real per-row causal softmax, evaluated exactly in the Scheme B
// client boundary, so it generalizes to any T without re-deriving an
// algebraic shortcut per sequence length.
//
...
// T=3 fixture (gsr_pos0_block0_t3_d63353abdc1a_52d046d1fcf0), same checkpoint
// and GSR FASTA as the frozen T=2 fixture -- only --tokens differs.
...
[[noreturn]] void usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides_scheme_b_general_attention --gpu N "
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        << "  \"schema_version\": 1,\n"
        << "  \"task\": \"FHE real-weight DNAGPT block-0 gate, Scheme B hybrid client-assisted CKKS, FIDESlib GPU, GENERAL causal attention (T>2)\",\n"
        << "  \"implementation_version\": \"t3-scheme-b-general-attention-v1\",\n"
