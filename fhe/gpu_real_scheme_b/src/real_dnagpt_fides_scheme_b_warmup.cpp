// Scheme B (hybrid client-assisted CKKS) GPU WARM-UP-PRELUDE prototype,
// forked from real_dnagpt_fides_scheme_b_profiled.cpp (itself forked from
// the frozen, hash-pinned single-shot gate real_dnagpt_fides_scheme_b.cpp).
// Do not edit either of those files to add this behavior; their SHA-256
// values are recorded as provenance in existing immutable evidence JSONs.
//
// Finding this investigates (docs/hybrid/tasks.md, 2026-07-27 profiling
// session, fhe_fides_real_d768_t2_full_profiled_scheme_b_a100_20260727.json):
// per-matmul-call cost is NOT uniform across the block's 24 calls -- the 6
// QKV-stage calls (first real linear algebra in the block) average ~42s/call
// while the 8 matmul_mlp_projection calls (last stage) average ~13.3s/call,
// a ~3.15x front-to-back gap, despite byte-identical BSGS shape (1024
// ciphertext-plaintext multiplications each) and despite both stages
// operating on similarly "fresh" low-level ciphertexts (2 levels consumed
// since their respective client-boundary resets -- checked directly, so a
// depth/level-cost explanation does not hold here).
//
// A caching fix that tried to exploit an assumed redundant-CPU-encoding
// explanation for this gap (real_dnagpt_fides_scheme_b_diagcache.cpp) was
// abandoned after 6 consecutive real-GPU crashes (Cuda failure ... out of
// memory, addr2line-confirmed inside Ciphertext::multPt ->
// Plaintext::adjustPlaintextToCiphertext -> RNSPoly::grow -> GPUmalloc)
// bracketing 2 clean same-session passes of the unmodified baseline --
// narrowed to something about reusing a GPU-resident Plaintext object across
// two separate multPt calls that this pinned FIDESlib build does not safely
// tolerate, mechanism not fully resolved at the CUDA/RNSPoly internals level.
// See docs/hybrid/tasks.md's 2026-07-27/28 investigation subsection for the
// full evidence trail. That approach is not reused here.
//
// This binary tests an entirely different, much lower-risk hypothesis: the
// gap is a CUDA/allocator/kernel warm-up effect (a well-documented class of
// phenomenon -- first cudaMalloc calls of a new size class, first-launch
// kernel JIT/module loading -- being slower than steady-state reuse), NOT
// anything about which specific C++ objects get reused. It adds one
// untimed (separately timed, not folded into encrypted_evaluation_seconds)
// warm-up pass immediately after context setup and before the real
// evaluate() call: one ciphertext-plaintext multiply, one ciphertext-
// ciphertext multiply, one EvalAdd, one giant-style single EvalRotate, and
// one baby-style hoisted EvalFastRotation batch, all at the real problem's
// exact SLOTS=4096/ring_dim=65536/depth=16 shapes, using a throwaway dummy
// ciphertext/plaintext that is discarded immediately after. Nothing in the
// real evaluate() call is touched: same matmul()/bsgs_inner_loop as the
// profiled baseline (byte-identical -- diffed against it), same packing,
// same BSGS_N1=32/BSGS_N2=32 split, same TOL=4e-2 oracle gate, same
// client-only decrypt boundary, same profiling bucket labels for direct
// comparison. If the hypothesis is correct, matmul_qkv_query/key/value's
// per-call cost in this run's evidence should drop toward
// matmul_mlp_projection's level; if not, this is reported as a negative
// result just as plainly, not tuned or hidden.
//
...
// final Decrypt, exactly as the original does. The warm-up pass never
// touches the secret key or any real fixture data -- it operates only on a
// throwaway all-zero dummy plaintext/ciphertext pair encrypted and decrypted
// nowhere.
...
[[noreturn]] void usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides_scheme_b_profiled --gpu N "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
            ...
            std::cout << "usage: real_dnagpt_fides_scheme_b_profiled --gpu N "
  ...
  public:
    ...
    EvaluationResult evaluate(const std::array<Ct, T>& encrypted_inputs,
        ...
        for (std::size_t token = 0; token < T; ++token) {
            ...
            const bool detail_this_call = (token == 0);
            const auto q_start = Clock::now();
            query[token] = timed_ct("matmul_qkv_query", [&] {
                return matmul(baby, fixture_.qkv[0], detail_this_call);
            });
...
std::string make_json(const Options& options, std::uint32_t ring,
                      ...
                      double setup_seconds, double warmup_seconds,
                      double encryption_seconds,
    ...
    out << "{\n"
        ...
        << "  \"implementation_version\": \"t2-scheme-b-warmup-v1\",\n"
        ...
        << "  \"label\": \"warm-up-prelude prototype: one untimed dummy pass (ct-pt multiply, ct-ct multiply, EvalAdd, giant EvalRotate, baby EvalFastRotation) at the real problem's exact shapes runs after context setup and before the real timed evaluate() call, to test whether the profiled baseline's QKV-vs-mlp_projection front-to-back cost gap is a CUDA allocator/kernel warm-up effect; same matmul()/bsgs_inner_loop and profiling buckets as the profiled baseline for direct comparison; not a caching or object-reuse prototype (that approach was abandoned as unsafe, see source header)\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        // Warm-up prelude (see header comment): exercise each primitive op
        // kind once, at the real problem's exact shapes, on a throwaway
        // dummy ciphertext/plaintext pair that touches no fixture data and
        // no secret key beyond the one Encrypt/Decrypt pair below (mirroring
        // what every other gate already does for its real input/output).
        // Timed separately -- never folded into encrypted_evaluation_seconds
        // -- so this is an honest prelude, not a way to hide cost.
        const auto warmup_start = Clock::now();
        {
            const std::vector<double> dummy_values(SLOTS, 0.0);
            Plaintext dummy_plain =
                cc->MakeCKKSPackedPlaintext(dummy_values, 1, 0, nullptr, SLOTS);
            Ct dummy_ct = cc->Encrypt(keys.publicKey, dummy_plain);

            Ct ct_pt_result = cc->EvalMult(dummy_ct, dummy_plain);
            Ct ct_ct_result = cc->EvalMult(dummy_ct, dummy_ct);
            Ct add_result = cc->EvalAdd(ct_pt_result, ct_ct_result);
            Ct giant_rotate_result =
                cc->EvalRotate(add_result, static_cast<std::int32_t>(BSGS_N1));

            std::vector<std::int32_t> baby_indices;
            baby_indices.reserve(BSGS_N1 - 1);
            for (std::size_t index = 1; index < BSGS_N1; ++index) {
                baby_indices.push_back(static_cast<std::int32_t>(index));
            }
            const auto baby_rotated = cc->EvalFastRotation(
                giant_rotate_result, baby_indices, cc->GetCyclotomicOrder(),
                nullptr);
            if (baby_rotated.size() != BSGS_N1 - 1) {
                throw std::runtime_error(
                    "warm-up: FIDESlib returned incomplete baby rotations");
            }
            cc->Synchronize();
        }
        const double warmup_seconds = elapsed_seconds(warmup_start);
        std::cout << "[warmup] seconds=" << warmup_seconds << '\n';
        ...
        const std::string evidence = make_json(
            ...
            warmup_seconds, encryption_seconds, evaluation_seconds, decrypt_seconds,
