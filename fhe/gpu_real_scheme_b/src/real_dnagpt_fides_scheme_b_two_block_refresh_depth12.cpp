// Depth-12 two-block composition fork. Parent wrapper SHA-256:
// 5cb82f81dc46efe2f4bfd7a1c9e9c1bf90cea3d629432b027b42e5df89093767.
// The included single-block depth-12 source is separately hash-pinned.
...
#include "real_dnagpt_fides_scheme_b_depth12.cpp"
#undef main
...
constexpr std::string_view FROZEN_SCHEME_B_SOURCE_SHA256 =
    "234e3659eb73685619fede1b95dcf637c72f5b504e54a3631dfb1ab4ce0d5397";
constexpr std::string_view ORIGINAL_SCHEME_B_SOURCE_SHA256 =
...
[[noreturn]] void two_block_usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides_scheme_b_two_block_refresh_depth12 --gpu N "
...
TwoBlockOptions parse_two_block_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
            ...
            std::cout
                << "usage: real_dnagpt_fides_scheme_b_two_block_refresh_depth12 "
...
std::string make_two_block_json(
    ...
    out << "{\n"
        ...
        << "  \"implementation_version\": "
           "\"t2-scheme-b-two-block-full-hidden-refresh-depth12-v1\",\n"
        ...
        << "  \"depth12_single_block_source_sha256\": \""
        << json_escape(options.frozen_source_sha256) << "\",\n"
        << "  \"frozen_single_block_parent_source_sha256\": \""
        << ORIGINAL_SCHEME_B_SOURCE_SHA256 << "\",\n"
    ...
    try {
        ...
        std::cout
            << (passed
                    ...
                    : "REAL_DNAGPT_FIDES_SCHEME_B_TWO_BLOCK_REFRESH_DEPTH12_FAIL")
            << '\n';
        return passed ? 0 : 5;
