...
// Real-weight DNAGPT block-0 gates on FIDESlib CKKS/CUDA, Scheme B (hybrid
// client-assisted CKKS).
//
// This is a Scheme B sibling of fhe/gpu_real_sigmoid/src/real_dnagpt_fides_sigmoid.cpp.
// It keeps the identical released dna_gpt0.1b_m fixture, CKKS packing layout,
// BSGS matmul, T=2 causal-attention algebraic identity, encrypted-input
// boundary, and 4e-2 dual accuracy gate. The only change is how the three
// nonlinearity classes (LayerNorm invsqrt x2, attention sigmoid, GELU x4) are
// evaluated:
//
//   Scheme A (fhe/gpu_real_sigmoid): each nonlinearity is a fixed-degree
//   Chebyshev polynomial approximation over a calibrated public domain,
//   evaluated homomorphically. This accumulates real multiplicative depth
//   that must survive to the end of the block (or later, across blocks).
//
//   Scheme B (this file): each nonlinearity is a pre-declared client
//   boundary. The data-owning client (who already holds the secret key to
...
//
// Security boundary: EncryptedEvaluator never stores a private key; it holds
// only a reference to a Client that owns the secret key and performs exactly
// the pre-declared cross_boundary() calls below. main() additionally performs
// one final Decrypt after the selected gate completes, exactly as in Scheme A.
...
// Scheme A uses depth 43 to survive four Chebyshev series (degrees 27, 15,
// 13, 15) without an intra-block bootstrap. Scheme B resets to level 0 at
// every nonlinearity via Client::cross_boundary, so only the *linear* segment
// between two boundaries needs to fit inside the budget. This starting value
// is a conservative estimate pending the first empirical level-trace run
// (see the "[stage]" prints); reduce it once real level maxima are measured,
// to stay inside the cheapest ring_dim bucket for this security level.
...
[[noreturn]] void usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides_scheme_b --gpu N --gate ln1|attention|full "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
            ...
            std::cout << "usage: real_dnagpt_fides_scheme_b --gpu N "
...
// the role of the data-owning client in the Scheme B protocol. The server
// (EncryptedEvaluator below) calls cross_boundary() at exactly the three
// pre-declared nonlinearity classes; every call is logged (name + wall time)
// so the evidence JSON carries an honest, complete round-trip count.
class Client {
  public:
    // KeyPair's copy constructor is deleted, so the client stores a reference
    // to the caller's keys (main() keeps `keys` alive for the whole process,
    // for the encrypted-input encryption before this and the final decrypt
    // after this).
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        ...
        << "  \"implementation_version\": \"t2-scheme-b-v2-batched-depth12\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        // cross_boundary() decrypt/exact-compute/re-encrypt calls inside
        // evaluate(), and the one final Decrypt below.
