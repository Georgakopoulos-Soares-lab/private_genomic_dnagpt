...
// T=103, CONFIG-3 "all-optimizations" variant: encrypted embeddings -> 12
// transformer blocks -> encrypted N-minus-A GSR classifier margin, composing
// the CPU-side diagonal-vector-cache per-block source instead of the plain
// depth-13 block. T=103 is the COMPLETE tokenized GSR prompt for this record
// (not a truncated prefix like the T=2 driver), so this is a real-task-
// semantics correctness/composition gate, not only a graph gate.
//
// This file is an ADDITIVE fork of the config-1 end-to-end driver
// real_dnagpt_fides_scheme_b_simd_full_t103_12blocks_head.cpp (pinned SHA-256
// below). Its composition logic -- one crypto context/key lineage, 12 blocks,
// eleven full-hidden-state inter-block refreshes, one block-11->head last-token
// refresh, and the GSR N-minus-A head -- is BYTE-FOR-BYTE identical to that
// parent. The ONLY difference is which per-block source it #includes: the
// CPU-diagonal-vector-cache block (..._cpudiagcache.cpp) rather than the plain
// depth-13 block. The cpudiagcache block is a strict superset of the depth-13
// block -- same EncryptedEvaluator/Client/Fixture/EvaluationResult/evaluate()
// symbol surface, same op counts and boundary counts -- differing only by an
// internal, class-private host-side std::vector<double> diagonal cache, so the
// #include-swap changes nothing about the composed encrypted graph.
//
// Fork discipline:
//   - includes the CPU-diagcache depth-13/digits-3/ring-65536 Token-SIMD B=8
//     T=103 single-block source unchanged (pinned SHA-256 below) and reuses its
...
//   config-1 12blocks_head e2e driver SHA-256 (this file's fork parent;
//   composition logic copied verbatim, per-block #include swapped):
//     355f1857bec77471192f3d81010f88b478d9b3707f63ebe5898526f99791a5da
//   cpudiagcache per-block source SHA-256 (included here, unchanged):
...
#define main real_dnagpt_fides_scheme_b_simd_full_t103_depth13_digits3_ring65536_cpudiagcache_main_unused
...
struct AllBlocksOptions {
    ...
    std::string cpudiagcache_source_sha256;
...
[[noreturn]] void all_blocks_usage_error(const std::string& message) {
    throw std::invalid_argument(
        ...
        "--cpudiagcache-source-sha256 SHA --two-block-refresh-source-sha256 SHA "
...
AllBlocksOptions parse_all_blocks_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
        ...
        } else if (arg == "--cpudiagcache-source-sha256") {
            options.cpudiagcache_source_sha256 = next();
            ...
            std::cout
                << "usage: "
                   ...
                   "cpudiagcache "
                   "--gpu N --fixture-dir PATH --output PATH "
                   "--cpudiagcache-source-sha256 SHA "
                   ...
                   "CPU-side diagonal-vector cache.\n";
            std::exit(0);
        } else {
    ...
    if (options.cpudiagcache_source_sha256 != PINNED_CPUDIAGCACHE_SOURCE_SHA256) {
        all_blocks_usage_error(
            "refusing changed cpudiagcache per-block parent source " +
            options.cpudiagcache_source_sha256);
    }
...
std::string make_all_blocks_json(
    ...
    out << "{\n"
        ...
        << "  \"task\": \"Released-weight DNAGPT complete 12-block backbone "
           ...
           "T=103 (full prompt), CPU-side diagonal-vector cache\",\n"
        << "  \"implementation_version\": "
           "\"t103-scheme-b-token-simd-all-12-blocks-head-cpudiagcache-v1\",\n"
        << "  \"scheme\": \"B (hybrid client-assisted CKKS)\",\n"
        << "  \"measured_at_utc\": \"" << utc_now() << "\",\n"
        << "  \"gate\": \"all_blocks_head_t103_cpudiagcache\",\n"
        ...
        << "  \"cpudiagcache_parent_source_sha256\": \""
        << json_escape(options.cpudiagcache_source_sha256) << "\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << evidence;
        std::cout << (passed ? "REAL_DNAGPT_FIDES_SCHEME_B_ALL_BLOCKS_HEAD_T103_CPUDIAGCACHE_PASS"
