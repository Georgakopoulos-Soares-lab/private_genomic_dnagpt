// Scheme B (hybrid client-assisted CKKS) GPU-side DIAGONAL-PLAINTEXT-CACHE
// fix prototype, forked from real_dnagpt_fides_scheme_b_profiled.cpp (itself
// forked from the frozen, hash-pinned single-shot gate
// real_dnagpt_fides_scheme_b.cpp). Do not edit either of those files to add
// this behavior; their SHA-256 values are recorded as provenance in existing
// immutable evidence JSONs.
//
// Finding this fixes (docs/hybrid/tasks.md, 2026-07-27 profiling session,
// fhe_fides_real_d768_t2_full_profiled_scheme_b_a100_20260727.json):
//
//   1. The 6 matmul() call-site stages (qkv_query/key/value,
//      attention_projection, mlp_fc, mlp_projection) account for 99.5% of
//      server_linear_algebra_seconds; everything else (LayerNorm, attention
//      score/softmax linear algebra, GELU pack/restore, residual adds) is
//      collectively under 0.5%.
//   2. Within one matmul() call, ciphertext-plaintext-multiply-and-
//      accumulate is ~100% of the call's internal cost; giant-step
//      EvalRotate/keyswitch is <0.1%; baby-step hoisted EvalFastRotation is
//      also negligible. Rotation is NOT the bottleneck at any BSGS split --
//      a stronger, more specific result than the already-ruled-out "retune
//      N1/N2" lever (CLAUDE.md), since it shows the entire rotation
//      operation class, not just its N1/N2 balance, is immaterial here.
//   3. Per-call cost is NOT uniform across the block's 24 matmul() calls:
//      the 6 QKV-stage calls average ~42s/call while the 8
//      matmul_mlp_projection calls average ~13.3s/call -- a ~3.15x
//      front-to-back gap despite byte-identical BSGS shape (1024
//      ciphertext-plaintext multiplications each). Root cause, confirmed by
//      reading real_dnagpt_fides_scheme_b.cpp's matmul()/bsgs_inner_loop:
//      there are only 12 DISTINCT D-by-D weight matrices in this block (3
//      QKV + 1 attention projection + 4 MLP fc + 4 MLP projection), each
//      used by BOTH of the T=2 tokens -- but every matmul() call
//      independently rebuilds and re-encodes its own 1024 BSGS diagonal
//      Plaintext objects from scratch via raw_plain()/
//      MakeCKKSPackedPlaintext, even on the token-1 call that uses the
//      IDENTICAL weight (and therefore identical diagonal values) as the
//      token-0 call for the same site. Pinned FIDESlib source
//      (api/CryptoContext.cpp:251-258, LoadPlaintext) shows this is
//      genuinely wasted work: `LoadPlaintext(Plaintext& pt)` returns
//      immediately if `pt->loaded` is already true, so re-using the SAME
//      Plaintext object (not merely an equal one) across a weight's two
//      token-calls lets FIDESlib's own lazy-load guard skip the
//      CPU-encode+GPU-upload+register step entirely on the second call --
//      only the ciphertext-side multPt (which genuinely differs per token)
//      still runs both times.
//
// The fix: EncryptedEvaluator memoizes each weight's 1024 diagonal
// Plaintext objects (keyed by the weight matrix's stable address in
// Fixture, which lives for the whole evaluate() call) on first use, and
// reuses the SAME objects on the weight's second token-call. This changes
// NOTHING about the encrypted computation: same packing, same
// BSGS_N1=32/BSGS_N2=32 split, same TOL=4e-2 oracle gate, same
// client-only decrypt boundary, same diagonal VALUES (only the Plaintext
// object identity is reused, not its content). This binary keeps the exact
// same profiling instrumentation as real_dnagpt_fides_scheme_b_profiled.cpp
// (same bucket names/call sites) so the two evidence files are directly,
// bucket-by-bucket comparable -- this is not a claim of speedup until that
// comparison is measured on real GPU hardware.
//
...
class EncryptedEvaluator {
  ...
  private:
    ...
    Ct matmul(const BabyRotations& baby, const Matrix& weight, bool detailed) {
        ...
        for (std::size_t giant = 0; giant < BSGS_N2; ++giant) {
            ...
            if (detailed) {
                ...
                inner = bsgs_inner_loop(baby, weight, giant, detailed);
                cc_->Synchronize();
    ...
    // Memoized per-weight BSGS diagonal plaintexts. Keyed by the weight
    // matrix's stable address (Fixture's Matrix objects live for the whole
    // evaluate() call, and every matmul() call site always passes the same
    // Fixture member by reference), NOT by a copy of its values -- this is
    // the mechanism that lets FIDESlib's own `LoadPlaintext` skip the
    // second token's CPU-encode+GPU-upload: reusing the SAME Plaintext
    // object (a cheap handle copy per multiply_plain call) means its
    // `loaded` flag, set true by token 0's use, is already true when token
    // 1 reuses it. See the header comment for the FIDESlib source citation.
    const std::vector<Plaintext>& cached_diagonal_plaintexts(
        const Matrix& weight, bool detailed) {
        const auto found = diagonal_cache_.find(&weight);
        if (found != diagonal_cache_.end()) {
            return found->second;
        }
        ...
        plaintexts.reserve(BSGS_N1 * BSGS_N2);
        for (std::size_t giant = 0; giant < BSGS_N2; ++giant) {
            for (std::size_t small = 0; small < BSGS_N1; ++small) {
                ...
                plaintexts.push_back(raw_plain(packed_values));
            }
        }
        if (detailed) {
            cc_->Synchronize();
            profiler_.add("diagonal_plaintext_build_detailed",
                          elapsed_seconds(build_start));
        } else {
            profiler_.add("diagonal_plaintext_build",
                          elapsed_seconds(build_start));
        }
        const auto [inserted, _] =
            diagonal_cache_.emplace(&weight, std::move(plaintexts));
        return inserted->second;
    }

    Ct bsgs_inner_loop(const BabyRotations& baby, const Matrix& weight,
                       std::size_t giant, bool detailed) {
        const std::vector<Plaintext>& plaintexts =
            cached_diagonal_plaintexts(weight, detailed);
        Ct inner;
        for (std::size_t small = 0; small < BSGS_N1; ++small) {
            const std::size_t diagonal = BSGS_N1 * giant + small;
            Ct term = multiply_plain(baby.values[small], plaintexts[diagonal]);
    ...
    // Keyed by weight-matrix address, not value: every matmul() call site
    // always passes the same Fixture member by reference, so the address is
    // a stable, correct cache key for the lifetime of one evaluate() call.
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        ...
        << "  \"implementation_version\": \"t2-scheme-b-diagcache-v1\",\n"
        ...
        << "  \"label\": \"diagonal-plaintext-cache fix prototype: memoizes each of the 12 distinct weight matrices' 1024 BSGS diagonal Plaintext objects, reused (same object identity, not just equal values) across the T=2 token-calls that share a weight, so FIDESlib's own LoadPlaintext lazy-load guard skips the second token's CPU-encode+GPU-upload; same profiling instrumentation and bucket names as the profiled baseline for direct comparison; not a context/key caching or serialization prototype\",\n"
