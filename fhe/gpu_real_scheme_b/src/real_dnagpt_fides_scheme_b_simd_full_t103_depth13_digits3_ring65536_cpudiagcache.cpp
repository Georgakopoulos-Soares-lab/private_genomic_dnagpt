...
std::string make_json(
    ...
    out << "{\n"
        ...
        << "  \"task\": \"Scheme B Token-SIMD T=103 "
           ...
           "released-weight DNAGPT block-0 gate, CPU-side "
           "diagonal-vector cache (reopened diagcache lever, scoped "
           "to avoid the abandoned GPU-resident-Plaintext-reuse "
           "crash)\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << (metrics.passed
                          ? "REAL_DNAGPT_TOKEN_SIMD_FULL_DEPTH13_DIGITS3_RING65536_CPUDIAGCACHE_GATE_PASS\n"
