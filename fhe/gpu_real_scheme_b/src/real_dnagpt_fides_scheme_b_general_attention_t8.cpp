...
// GENERAL causal-attention prototype at T=8.
//
// Forked from real_dnagpt_fides_scheme_b_general_attention.cpp (the T=3
// anchor for this design, now itself frozen -- never edit that file; the
// original T=2 file real_dnagpt_fides_scheme_b.cpp is likewise frozen).
// The attention/softmax circuit itself is fully general in T already (see
// the T=3 file's design comment and docs/hybrid/tasks.md's 2026-07-28
// "general causal attention" entry), so no algorithmic change was needed
// to move from T=3 to T=8. This file exists to empirically confirm the
// O(T)/O(T^2) round-trip/score-count growth the T=3 design predicted, at
// a second data point, before any speed optimization or further scaling.
//
// Differences from the T=3 file: T itself, the pinned fixture manifest
// hash, and -- added after a first T=8 attempt crashed with
// "malloc(): unaligned tcache chunk detected" (glibc heap corruption)
// after ~28 minutes with NO diagnostic output, because std::cout was
// fully buffered when redirected to a log file and the crash never
// flushed it -- every existing "[stage]"/"[context]" print now uses
// std::endl instead of '\n' to force a flush, plus one NEW per-(row,col)
// "[stage] attention row=.. col=.." print inside the causal-attention
// loop (the T=3 file only logs once per record() checkpoint, i.e. before
// and after the entire attention block, which cannot localize a crash to
// a specific row/column). This is debug instrumentation for crash
// localization, not an algorithmic change.
//
// Design (see docs/hybrid/tasks.md's 2026-07-28 "general causal attention"
// entry for the full derivation and the numpy proof this was checked
// against):
//
//   For causal row i (query token i attends to key/value tokens 0..i):
//     - row 0 is trivial: context[0] = value[0] (softmax over one element).
//     - row i>=1 needs softmax over the i+1 raw scores s_i0..s_ii, per head.
//       Reusing the T=2 file's own algebraic reduction (context = v0 +
//       w*(v1-v0), which folds the j=0 weight into an implicit "1 minus the
//       rest" term instead of materializing it), context[i] = value[0] +
//       sum_{j=1}^{i} weight_ij * (value[j] - value[0]), where weight_ij =
//       exp(s_ij - max_j s_i*) / sum_j exp(s_ij - max_j s_i*) is the exact
//       (not polynomial-approximated) softmax weight.
//     - one Scheme B client boundary call per (row i, column j=1..i), each
//       batching all twelve heads into a single physical round trip (the
//       same "many logical values in one ciphertext" batching already
//       adopted for the T=2 attention sigmoid and the GELU chunks). Total
//       round trips for the attention block = sum_{i=1}^{T-1} i =
//       T*(T-1)/2 -- for T=2 this is exactly 1, matching the frozen file's
//       own round-trip count, confirming this is a strict generalization,
//       not a different algorithm that happens to agree at T=2.
//     - raw per-head scores are carried to the boundary in a purpose-built
//       packing, disjoint from the head's real HEAD_DIM=64-wide value
//       span: head h's row-i scores occupy slots
//       [h*SCORE_SLOT_STRIDE .. h*SCORE_SLOT_STRIDE+i] within each
//       PACK_WIDTH copy (SCORE_SLOT_STRIDE=T is comfortably inside the
//       256 slack slots per copy left over after D=768 real dimensions).
//       The client reads that slice, computes the exact softmax, and
//       writes back a value broadcast across the head's real 64-slot span
//       (so it can multiply elementwise against value[j]-value[0]) --
//       this is the same read-one-place/write-another freedom the existing
//       Client::cross_boundary_impl already grants any transform; no
//       change to cross_boundary_impl's decrypt/encrypt mechanics itself.
//
// Cost, honestly disclosed, not tuned away: attention_score() calls grow
// from HEADS*2 (T=2) to HEADS*(T*(T+1)/2 - 1) (sum over rows 1..T-1 of
// i+1 causal columns) -- this is the same O(T^2) causal-attention score
// count any plaintext transformer pays, not a Scheme B-specific inflation.
// Round trips grow from 1 to T*(T-1)/2, likewise inherent to needing one
// real softmax per non-trivial row rather than a single per-block identity.
//
// Everything else -- BSGS matmul, encrypted-input boundary, LayerNorm/GELU
// client boundaries, the 4e-2 dual accuracy gate -- is untouched from the
// frozen file.
...
// Per-head reserved width, within each PACK_WIDTH copy, for carrying a row's
// raw causal scores to the client boundary. Needs only >= T (max causal row
// length); HEADS*SCORE_SLOT_STRIDE must stay inside the D..PACK_WIDTH slack
// (768..1024, 256 slots) shared by all twelve heads.
constexpr std::size_t SCORE_SLOT_STRIDE = T;
static_assert(HEADS * SCORE_SLOT_STRIDE <= PACK_WIDTH - D,
             "packed causal-score reservation must not collide with the "
             "D real dimensions any head span also occupies");
// See MULT_DEPTH comment below: same starting budget as the frozen T=2
// file. The general attention path adds one extra multiply_plain per causal
// score (the one-hot isolation) versus the frozen file's per-head masking;
// this is bounded, not applied along the ln1->ln2 chain that governs the
// deepest overall path, so the frozen file's depth-16 estimate is reused as
// this prototype's starting point pending its own empirical level trace.
...
// T=8 fixture (gsr_pos0_block0_t8_d63353abdc1a_52d046d1fcf0), same checkpoint
// and GSR FASTA as the frozen T=2/T=3 fixtures -- only --tokens differs.
...
[[noreturn]] void usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides_scheme_b_general_attention_t8 --gpu N "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
            ...
            std::cout << "usage: real_dnagpt_fides_scheme_b_general_attention "
...
// Output-packing group size: the T=2/T=3 files pack every token's own
// PACK_WIDTH-wide copy slot into ONE final ciphertext, implicitly assuming
// T<=COPIES=4 (there are only COPIES physical copies to address). T=8
// exceeds that silently-assumed ceiling -- the first T=8 attempt crashed
// with heap corruption inside output_block_plain's std::fill_n writing
// past the end of a SLOTS-sized vector for token>=COPIES (see
// docs/hybrid/tasks.md's 2026-07-29 "T=8 output-packing ceiling" entry for
// the full diagnosis, including the flushed per-(row,col) logging that
// localized the crash to right after the attention circuit -- which itself
// completed all 28 round trips correctly). Fix: group tokens into
// ceil(T/COPIES) chunks of up to COPIES tokens each; one final ciphertext
// per chunk, addressed by the token's position WITHIN its chunk rather
// than its raw index. This is the only place PACK_WIDTH/COPIES's role as
// an output-addressing constraint (as opposed to a packing-redundancy
// constant) is fixed; the attention circuit itself is untouched.
...
class Client {
  public:
    ...
    // Reads `row_length` raw scores per head from slots
    // [head*read_stride .. head*read_stride+row_length-1] (the purpose-built
    // score packing, disjoint from any head's real value dimensions), and
    // writes the softmax weight at `select_index` broadcast across the
    // head's real `write_span`-wide value span (so it can multiply
    // elementwise against a value/value-delta ciphertext). One decrypt, one
    // encrypt, all HEADS heads batched into this single physical round trip
    // -- the same "many logical values, one ciphertext" batching already
    // adopted for the T=2 file's attention_sigmoid_heads_batched and GELU
    // chunk boundaries.
    Ct cross_boundary_softmax_select(const std::string& name,
        ...
        return cross_boundary_impl(
            name, ciphertext,
            [row_length, select_index, heads, read_stride,
             write_span](std::vector<double>& values) {
                std::vector<double> transformed(SLOTS, 0.0);
                for (std::size_t copy = 0; copy < COPIES; ++copy) {
                    const std::size_t block_start = copy * PACK_WIDTH;
                    for (std::size_t head = 0; head < heads; ++head) {
                        const std::size_t read_start =
                            block_start + head * read_stride;
...
class EncryptedEvaluator {
  public:
    ...
    EvaluationResult evaluate(const std::array<Ct, T>& encrypted_inputs,
        ...
        for (std::size_t row = 1; row < T; ++row) {
            ...
            // isolated to its own reserved slot via a one-hot plaintext mask
            // (disjoint from the head's real 64-wide value span).
            ...
            for (std::size_t head = 0; head < HEADS; ++head) {
                ...
                for (std::size_t col = 0; col < row_length; ++col) {
                    ...
                    const Ct isolated = multiply_plain(
                        raw_score,
                        one_hot_plain(head * SCORE_SLOT_STRIDE + col));
            ...
            // all HEADS heads batched into each single round trip; j=0's
            // weight is never materialized (folded into the v0+delta form
            // below, exactly mirroring the T=2 file's own reduction).
            ...
            for (std::size_t col = 1; col <= row; ++col) {
                Ct weight_broadcast = client_.cross_boundary_softmax_select(
                    ...
                    packed_scores, row_length, col, HEADS, SCORE_SLOT_STRIDE,
                    HEAD_DIM, HEADS);
                ...
                // T=8-only crash-localization instrumentation (not present in
                // the T=3 anchor): flushed immediately so a crash mid-loop
                // still leaves a trace of the last completed (row,col) in
                // the log, unlike the T=3 file's stage-level-only prints
                // which are lost to stdout buffering on a hard crash.
  ...
  private:
    ...
    // One-hot plaintext isolating a single reserved score slot (per copy),
    // used only to carry raw causal scores to the softmax client boundary --
    // disjoint from any head's real value dimensions.
    Plaintext one_hot_plain(std::size_t slot_within_copy) {
    ...
    // by T (see the OUTPUT_GROUPS comment for why the frozen T=2/T=3 files'
    // token-indexed version silently assumed T<=COPIES and crashed at T=8).
...
// many output groups finish() produced (see OUTPUT_GROUPS) -- unlike the
// T=2/T=3 files, this no longer reads directly from one T*PACK_WIDTH raw
// decrypted slot layout, since T=8 needs more than one such ciphertext.
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        << "  \"schema_version\": 1,\n"
        << "  \"task\": \"FHE real-weight DNAGPT block-0 gate, Scheme B hybrid client-assisted CKKS, FIDESlib GPU, GENERAL causal attention (T>2), T=8 growth-trend check\",\n"
        << "  \"implementation_version\": \"t8-scheme-b-general-attention-v1\",\n"
        ...
        << "    \"client_boundary_batching\": \"attention: 12 heads in one ciphertext per (row,column) softmax boundary call; GELU: four 768-value chunks in one ciphertext per token; LayerNorm remains per token\",\n"
...
int main(int argc, char** argv) {
    try {
        ...
        // One decrypt per output group (see OUTPUT_GROUPS): T=8 needs
        // ceil(8/4)=2 final ciphertexts, unlike T=2/T=3's single decrypt.
