// Scheme B (hybrid client-assisted CKKS) GPU-side PROFILING prototype,
// forked from real_dnagpt_fides_scheme_b.cpp (the frozen, hash-pinned
// single-shot gate). Do not edit that file to add this behavior; its
// SHA-256 is recorded as provenance in existing immutable evidence JSONs.
//
// Purpose: docs/hybrid/tasks.md's 2026-07-27 caching/serialization sessions
// closed setup/key reuse as a speed lever (< 2% of wall time even in the
// worst measured case). The remaining cost is the server-side GPU linear
// algebra inside one block-0 "full" gate: ~384s out of ~386s of encrypted
// evaluation (fhe_fides_real_d768_t2_full_serialized_reader_scheme_b_a100_20260727).
// This binary finds out where that time actually goes, using real
// FIDESlib/CUDA-synchronized wall-clock timing (Synchronize() barriers), not
// a static call-count model alone -- though the call counts below ARE
// verified against the existing evidence JSON's
// explicit_ciphertext_plaintext_multiplications=24666 and
// explicit_ciphertext_ciphertext_multiplications=33 fields
// (fhe_fides_real_d768_t2_block0_batched_scheme_b_a100_20260726.json), which
// this file's own instrumentation reproduces exactly (see
// test_profiling_contract.py) as a cross-check that no correctness or
// operation-count behavior changed.
//
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        ...
        << "  \"implementation_version\": \"t2-scheme-b-profiled-v1\",\n"
        ...
        << "  \"label\": \"profiling-only prototype: Synchronize()-bracketed wall-clock timing of matmul call sites (all 24), one fully detailed representative matmul call (giant-step breakdown), and every other linear-algebra stage; not a caching or serialization prototype; adds barrier overhead not present in the unmodified single-shot/cached gates, disclosed as unaccounted_seconds below\",\n"
