#!/usr/bin/env bash
# Orchestrator for the T=8 general causal-attention growth-trend checkpoint
# (real_dnagpt_fides_scheme_b_general_attention_t8): waits for host load +
# GPU capacity, then runs the requested gate (ln1, attention, or full)
# against the real T=8 fixture unattended.
#
# Mirrors wait_and_run_scheme_b_general_attention.sh's capacity-wait logic
# exactly. Runs directly on the Brev host (not inside a container) since it
# needs host-level `uptime`/`nvidia-smi` and drives
# launch_brev_scheme_b_general_attention_t8.sh, which itself launches the
...
readonly FIXTURE_SUBDIR="${SCHEME_B_FIXTURE_SUBDIR:-real_fixture/gsr_pos0_block0_t8_d63353abdc1a_52d046d1fcf0}"
readonly IMAGE="${SCHEME_B_IMAGE:-dnagpt-fideslib:786c-asymfix2}"
readonly RUN_TAG="${SCHEME_B_GENERAL_ATTENTION_T8_RUN_TAG:?set SCHEME_B_GENERAL_ATTENTION_T8_RUN_TAG (must contain _scheme_b_ and _general_attention_t8_)}"
readonly GATE="${SCHEME_B_GENERAL_ATTENTION_T8_GATE:-attention}"
...
readonly ORCHESTRATOR_LOG="${SOURCE_DIR}/scheme_b_general_attention_t8_orchestrator.log"
...
run_general_attention_t8() {
  local tag="$1" gpu="$2"
  local done_file="${SOURCE_DIR}/${tag}.done"
  log "launching general-attention-t8 run tag=${tag} gpu=${gpu} gate=${GATE}"
  if ! "${SOURCE_DIR}/launch_brev_scheme_b_general_attention_t8.sh" "${gpu}" "${REMOTE_ROOT}" \
...
main() {
  log "general-attention-t8 orchestrator starting (pid $$) tag=${RUN_TAG} gate=${GATE}"
  ...
  while true; do
    ...
    run_general_attention_t8 "${RUN_TAG}" "${gpu}"
    rc=$?
    if [[ "${rc}" -eq 99 ]]; then
      sleep 30
      continue
    ...
    if [[ "${rc}" -ne 0 ]]; then
      log "FATAL: general-attention-t8 run failed with exit ${rc}; stopping (manual investigation needed)"
      exit "${rc}"
    fi
    log "general-attention-t8 run PASSED"
    break
  done
  log "SCHEME B GENERAL ATTENTION T8 RUN COMPLETE"
}
