...
if [[ $# -ne 6 ]]; then
  echo "usage: $0 PHYSICAL_GPU REMOTE_ROOT SOURCE_SUBDIR FIXTURE_SUBDIR IMAGE_TAG REPEATS(>=2)" >&2
  exit 2
fi
...
if [[ "${RUN_TAG}" != *"_scheme_b_"* ]] || [[ "${RUN_TAG}" != *"_cached_"* ]]; then
  echo "[FATAL] cached Scheme B evidence tag must contain both _scheme_b_ and _cached_" >&2
  exit 2
fi
...
export IMAGE_TAG REPEATS RUN_TAG LOG DONE PREFLIGHT_NOTE
nohup sh -c '
  docker run --rm \
    ...
    --entrypoint /bin/bash \
    "${IMAGE_TAG}" \
    "/work/${SOURCE_SUBDIR}/run_scheme_b_cached.sh" \
    0 \
    "${REPEATS}" \
    "/work/${FIXTURE_SUBDIR}" \
