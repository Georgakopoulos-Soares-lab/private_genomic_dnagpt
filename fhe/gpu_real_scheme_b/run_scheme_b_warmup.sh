...
if [[ $# -ne 3 ]]; then
  ...
  echo "OUTPUT_JSON filename must contain both _scheme_b_ and _warmup_" >&2
...
readonly OUTPUT="$3"
readonly BINARY="${BUILD_DIR}/real_dnagpt_fides_scheme_b_warmup"

if [[ "$(basename "${OUTPUT}")" != *"_scheme_b_"* ]] || \
   [[ "$(basename "${OUTPUT}")" != *"_warmup_"* ]]; then
  echo "[FATAL] warmup Scheme B evidence filename must contain both _scheme_b_ and _warmup_" >&2
  exit 2
fi
if [[ -e "${OUTPUT}" ]]; then
...
source_sha="$(sha256sum "${SCRIPT_DIR}/src/real_dnagpt_fides_scheme_b_warmup.cpp" | cut -d' ' -f1)"
