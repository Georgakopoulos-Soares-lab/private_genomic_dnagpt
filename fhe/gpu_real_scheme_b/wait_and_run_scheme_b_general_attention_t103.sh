#!/usr/bin/env bash
# Orchestrator for the T=103 chunked/streaming-softmax checkpoint
# (real_dnagpt_fides_scheme_b_general_attention_t103): waits for host load +
# GPU capacity, then runs the requested gate (ln1, attention, or full)
# against the real T=103 fixture unattended.
#
# Mirrors wait_and_run_scheme_b_general_attention_t32.sh's capacity-wait
...
# launch_brev_scheme_b_general_attention_t103.sh, which itself launches the
...
readonly RUN_TAG="${SCHEME_B_GENERAL_ATTENTION_T103_RUN_TAG:?set SCHEME_B_GENERAL_ATTENTION_T103_RUN_TAG (must contain _scheme_b_ and _general_attention_t103_)}"
readonly GATE="${SCHEME_B_GENERAL_ATTENTION_T103_GATE:-attention}"
...
readonly ORCHESTRATOR_LOG="${SOURCE_DIR}/scheme_b_general_attention_t103_orchestrator.log"
...
run_general_attention_t103() {
  local tag="$1" gpu="$2"
  local done_file="${SOURCE_DIR}/${tag}.done"
  log "launching general-attention-t103 run tag=${tag} gpu=${gpu} gate=${GATE}"
  if ! "${SOURCE_DIR}/launch_brev_scheme_b_general_attention_t103.sh" "${gpu}" "${REMOTE_ROOT}" \
...
main() {
  log "general-attention-t103 orchestrator starting (pid $$) tag=${RUN_TAG} gate=${GATE}"
  ...
  while true; do
    ...
    run_general_attention_t103 "${RUN_TAG}" "${gpu}"
    rc=$?
    if [[ "${rc}" -eq 99 ]]; then
      sleep 30
      continue
    ...
    if [[ "${rc}" -ne 0 ]]; then
      log "FATAL: general-attention-t103 run failed with exit ${rc}; stopping (manual investigation needed)"
      exit "${rc}"
    fi
    log "general-attention-t103 run PASSED"
    break
  done
  log "SCHEME B GENERAL ATTENTION T103 RUN COMPLETE"
}
