"""Fail-closed contract for the additive T=103 Token-SIMD depth-8 fork."""
...
SOURCE = ROOT / "src"
BASE = SOURCE / "real_dnagpt_fides_scheme_b_simd_full_t103.cpp"
DEPTH8 = SOURCE / "real_dnagpt_fides_scheme_b_simd_full_t103_depth8.cpp"

BASE_SHA256 = "97727a5bb2145b32749b746c3fc1b576811dfcc3222088278962b029fc1aea4b"
DEPTH8_SHA256 = "58c257522eefe7a928e6efaa486af6dd3ac53c289fc07878bb41d5d5f9350882"

REPLACEMENTS = (
    (
        "// Additive complete Token-SIMD block-0 gate for Scheme B.",
        "// Additive depth-8 complete Token-SIMD block-0 gate for Scheme B.",
    ),
    (
        "//   direct parent: passing T=103 Token-SIMD linear micro-gate",
    ...
    (
        'std::cout << "Token-SIMD T=103 complete block-0 gate\\n";',
        'std::cout << "Token-SIMD T=103 depth-8 complete block-0 gate\\n";',
    ),
    (
        '<< "  \\"task\\": \\"Scheme B Token-SIMD T=103 complete "',
        '<< "  \\"task\\": \\"Scheme B Token-SIMD T=103 depth-8 complete "',
    ),
    (
        '"\\"t103-scheme-b-token-simd-full-v1\\",\\n"',
    ...
    (
        '"REAL_DNAGPT_TOKEN_SIMD_FULL_GATE_FAIL\\n"',
...
class TokenSimdFullT103Depth8ContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.base = BASE.read_text(encoding="utf-8")
        cls.depth8 = DEPTH8.read_text(encoding="utf-8")

    def test_parent_and_fork_hashes_are_pinned(self) -> None:
        self.assertEqual(sha256(BASE), BASE_SHA256)
        self.assertEqual(sha256(DEPTH8), DEPTH8_SHA256)
    ...
    def test_fork_changes_only_declared_identity_and_depth_fields(self) -> None:
        ...
        self.assertEqual(self.depth8, expected)
    ...
    def test_run_script_pins_source_parent_and_fixture(self) -> None:
        ...
        for fragment in (
            ...
            "fixture_t103.sha256",
            'if [[ -e "${OUTPUT}" ]]',
            "_simd_full_t103_depth8_",
        ):
            self.assertIn(fragment, run)

    def test_launch_and_wait_scripts_keep_capacity_and_vram_gates(self) -> None:
        launch = (ROOT / "launch_brev_scheme_b_simd_full_t103_depth8.sh").read_text(
            encoding="utf-8"
        )
        wait = (ROOT / "wait_and_run_scheme_b_simd_full_t103_depth8.sh").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("SCHEME_B_SIMD_FULL_DEPTH8_RUN_TAG", wait)
        for script in (launch, wait):
            self.assertIn("_simd_full_t103_depth8_", script)
            self.assertNotIn("run_scheme_b_simd_full_t103.sh", script)
            self.assertNotIn("launch_brev_scheme_b_simd_full_t103.sh", script)

    def test_scripts_parse_and_are_executable(self) -> None:
        for name in (
            "run_scheme_b_simd_full_t103_depth8.sh",
            "launch_brev_scheme_b_simd_full_t103_depth8.sh",
            "wait_and_run_scheme_b_simd_full_t103_depth8.sh",
        ):
            path = ROOT / name
