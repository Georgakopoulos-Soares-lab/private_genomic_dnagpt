"""Validate the frozen public domains and T=2 numerator-first schedule locally.

This is a plaintext preflight, not FHE evidence. It uses NumPy's Chebyshev
interpolant at the exact degrees/domains compiled into real_dnagpt_fides.cpp.
"""
...
def main() -> None:
    ...
    exponential = cheb(np.exp, DOMAINS["exp"])
    reciprocal = cheb(lambda value: 1.0 / value, DOMAINS["reciprocal"])
    gelu = cheb(
        lambda value: 0.5
        * value
    ...
    exp_delta = exponential(delta)
    denominator = 1.0 + exp_delta
    context[:, 1, :] = (
        value[:, 0, :] + exp_delta[:, None] * value[:, 1, :]
    ) * reciprocal(denominator)[:, None]
    ...
    bounds = {
        ...
        "attention_score_delta_s1_minus_s0": DOMAINS["exp"][:2],
        "attention_denominator": DOMAINS["reciprocal"][:2],
