// Real-weight DNAGPT block-0 gates on FIDESlib CKKS/CUDA.
//
...
constexpr double ATTN_DENOM_LO = 1.0;
constexpr double ATTN_DENOM_HI = 2.05;
...
constexpr std::size_t DEG_RECIPROCAL = 13;
...
[[noreturn]] void usage_error(const std::string& message) {
    throw std::invalid_argument(
        message +
        "\nusage: real_dnagpt_fides --gpu N --gate ln1|attention|full "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu") {
            ...
            std::cout << "usage: real_dnagpt_fides --gpu N "
  ...
  public:
    EncryptedEvaluator(Cc context, const Fixture& fixture)
        ...
        exp_coefficients_ = cc_->GetChebyshevCoefficients(
            exponential, ATTN_DELTA_LO, ATTN_DELTA_HI, DEG_EXP);
        reciprocal_coefficients_ = cc_->GetChebyshevCoefficients(
            reciprocal, ATTN_DENOM_LO, ATTN_DENOM_HI, DEG_RECIPROCAL);
        gelu_coefficients_ =
    ...
    EvaluationResult evaluate(const std::array<Ct, T>& encrypted_inputs,
        ...
        for (std::size_t head = 0; head < HEADS; ++head) {
            ...
            Ct exponential = cc_->EvalChebyshevSeries(
                delta, exp_coefficients_, ATTN_DELTA_LO, ATTN_DELTA_HI);
            Ct denominator = cc_->EvalAdd(exponential, 1.0);
            Ct inverse = cc_->EvalChebyshevSeries(
                denominator, reciprocal_coefficients_, ATTN_DENOM_LO,
                ATTN_DENOM_HI);

            // Numerator first after an encrypted shift by score(source=0):
            // (v0 + exp(s1-s0)*v1) / (1 + exp(s1-s0)).
            Ct weighted_v1 = multiply(exponential, value_heads[head][1]);
            Ct numerator = cc_->EvalAdd(value_heads[head][0], weighted_v1);
            Ct head_context = multiply(numerator, inverse);
            context[1] =
...
std::string make_json(const Options& options, std::uint32_t ring,
    ...
    out << "{\n"
        << "  \"schema_version\": 1,\n"
        << "  \"task\": \"FHE real-weight DNAGPT block-0 gate, FIDESlib GPU, final-only decrypt\",\n"
        ...
        << ",\n"
        << "    \"attention_schedule\": \"numerator-first, encrypted source-0 shift, T=2 specialization\",\n"
        ...
        << ", " << ATTN_DELTA_HI << "],\n"
        << "      \"attention_denominator\": [" << ATTN_DENOM_LO << ", "
        << ATTN_DENOM_HI << "],\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << evidence;
        std::cout << (metrics.passed ? "REAL_DNAGPT_FIDES_GATE_PASS"
                                    : "REAL_DNAGPT_FIDES_GATE_FAIL")
                  << '\n';
